...
landscape')
    ...
    if 'hwp_cur_shape' in args:
        ...
        invest_core.rasterAdd(args['tot_C_cur'].GetRasterBand(1),
                              ...
                              args['tot_C_cur'].GetRasterBand(1))
        print('finished HWP storage for the current landscape')

    if 'lulc_fut' in args:
        #calculate carbon storage for the future landscape if it exists
        print('calculating carbon storage for future landscape')
        calculateCarbonStorage(pools, args['lulc_fut'].GetRasterBand(1),
                               args['tot_C_fut'].GetRasterBand(1))
        print('finished calculating carbon storage for future \
landscape')
        ...
        if 'c_hwp_fut' in args:
            print('calculating HWP storage for future landscape')
...
def valuation(args):
    ...
    print('constructing valuation formula')
    ...
    print('finished constructing valuation formula')

    print('starting valuation of each pixel')
    invest_core.vectorize1ArgOp(args['sequest'].GetRasterBand(1), valueOp,
                                args['value_seq'].GetRasterBand(1))
    print('finished valuation of each pixel')
