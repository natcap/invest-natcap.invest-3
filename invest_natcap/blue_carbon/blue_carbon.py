...
def execute(args):
    ...
    # half_life = raster_utils.get_lookup_from_csv(
    ...
    # # validating data
    # nodata_lulc = set([raster_utils.get_nodata_from_uri(
    ...
    # cell_size = set([raster_utils.get_cell_size_from_uri(
    #     lulc_uri_dict[k]) for k in lulc_uri_dict])
    # if len(cell_size) == 1:
    ...
    veg_trans_acc_dict = vars_dict['veg_trans_acc_dict']
    acc_bio_name = vars_dict['acc_bio_name']
    acc_soil_name = vars_dict['acc_soil_name']
    trans_dis_dict = vars_dict['trans_dis_dict']
    dis_soil_name = vars_dict['dis_soil_name']
    gdal_type_carbon = vars_dict['gdal_type_carbon']
    cell_size = vars_dict['cell_size']
    half_life = vars_dict['half_life']

    # pp.pprint(veg_trans_acc_dict[0]['acc_bio'].keys())
    # return
    ...
    carbon_name = vars_dict['carbon_name']
    acc_soil_name = vars_dict['acc_soil_name']
    acc_bio_name = vars_dict['acc_bio_name'] = "acc_bio"
    dis_bio_name = vars_dict['dis_bio_name']
    dis_soil_name = vars_dict['dis_soil_name']
    veg_litter_name = vars_dict['veg_litter_name']
    gain_name = vars_dict['gain_name']
    loss_name = vars_dict['loss_name']
    ...
    for veg_type in veg_type_list:
        ...
        this_veg_stock_soil_uri = vars_dict['veg_stock_soil_name'] % (
            this_year, veg_type)
        this_veg_stock_bio_uri = vars_dict['veg_stock_bio_name'] % (
            this_year, veg_type)
    ...
    for this_year, next_year in zip(
        ...
        this_total_acc_soil_uri = vars_dict['this_total_acc_soil_name'] % (
            this_year, next_year)
        this_total_acc_bio_uri = vars_dict['this_total_acc_bio_name'] % (
            this_year, next_year)
        this_total_dis_soil_uri = vars_dict['this_total_dis_soil_name'] % (
            this_year, next_year)
        this_total_dis_bio_uri = vars_dict['this_total_dis_bio_name'] % (
            this_year, next_year)
        ...
        for veg_type in veg_type_list:
            ...
            this_veg_litter_uri = vars_dict[
                'veg_litter_name'] % (this_year, veg_type)

            # disturbance and accumulation URI's
            this_veg_acc_bio_uri = vars_dict['veg_acc_bio_name'] % (
                this_year, next_year, veg_type)
            this_veg_acc_soil_uri = vars_dict['veg_acc_soil_name'] % (
                this_year, next_year, veg_type)
            this_veg_dis_bio_uri = vars_dict['veg_dis_bio_name'] % (
                this_year, next_year, veg_type)
            this_veg_dis_soil_uri = vars_dict['veg_dis_soil_name'] % (
                this_year, next_year, veg_type)

            # transition adjusted URI's
            this_veg_adj_acc_bio_uri = vars_dict['veg_adj_acc_bio_name'] % (
                this_year, next_year, veg_type)
            this_veg_adj_acc_soil_uri = vars_dict['veg_adj_acc_soil_name'] % (
                this_year, next_year, veg_type)
            this_veg_adj_dis_bio_uri = vars_dict['veg_adj_dis_bio_name'] % (
                this_year, next_year, veg_type)
            this_veg_adj_dis_soil_uri = vars_dict['veg_adj_dis_soil_name'] % (
                this_year, next_year, veg_type)

            # emission URI's
            this_veg_em_bio_uri = vars_dict['veg_em_bio_name'] % (
                this_year, next_year, veg_type)
            this_veg_em_soil_uri = vars_dict['veg_em_soil_name'] % (
                this_year, next_year, veg_type)

            # emission adjusted URI's
            this_veg_adj_em_dis_bio_uri = vars_dict[
                'veg_adj_em_dis_bio_name'] % (this_year, next_year, veg_type)
            this_veg_adj_em_dis_soil_uri = vars_dict[
                'veg_adj_em_dis_soil_name'] % (this_year, next_year, veg_type)
            ...
            for name, uri in [
                (vars_dict['veg_acc_bio_name'],
                    vars_dict['this_veg_acc_bio_uri']),
                (vars_dict['veg_acc_soil_name'],
                    vars_dict['this_veg_acc_soil_uri']),
                (vars_dict['veg_dis_bio_name'],
                    vars_dict['this_veg_dis_bio_uri']),
                (vars_dict['veg_dis_soil_name'],
                    vars_dict['this_veg_dis_soil_uri']),
                (vars_dict['veg_adj_acc_bio_name'],
                    vars_dict['this_veg_adj_acc_bio_uri']),
                (vars_dict['veg_adj_acc_soil_name'],
                    vars_dict['this_veg_adj_acc_soil_uri']),
                (vars_dict['veg_adj_dis_bio_name'],
                    vars_dict['this_veg_adj_dis_bio_uri']),
                (vars_dict['veg_adj_dis_soil_name'],
                    vars_dict['this_veg_adj_dis_soil_uri']),
                (vars_dict['veg_em_bio_name'],
                    vars_dict['this_veg_em_bio_uri']),
                (vars_dict['veg_em_soil_name'],
                    vars_dict['this_veg_em_soil_uri']),
                (vars_dict['veg_adj_em_dis_bio_name'],
                    vars_dict['this_veg_adj_em_dis_bio_uri']),
                (vars_dict['veg_adj_em_dis_soil_name'],
                    vars_dict['this_veg_adj_em_dis_soil_uri'])]:
    ...
    for veg_type in veg_type_list:
        this_veg_litter_uri = os.path.join(
            workspace_dir, veg_litter_name % (lulc_years[-1], veg_type))
    ...
    for i, this_year in enumerate(lulc_years):
        for next_year in (lulc_years+[analysis_year])[i+1:]:
            ...
            total_seq_uri = os.path.join(
                workspace_dir,
                vars_dict['net_sequestration_name'] % (this_year, next_year))
