...
class TestInvestCarbonCore(unittest.TestCase):
    def test_carbon_core_nodata(self):
        ...
        for xDim, yDim, inNoData, outNodata in [(1, 1, 255, 1.0),
            ...
            output_folder = './data/test_out/Carbon'
            if not os.path.exists(output_folder):
                os.makedirs(output_folder)
            lulc = driver.Create(os.path.join(output_folder,'test_blank_input'), xDim,
            ...
            tot_C_cur = driver.Create(os.path.join(output_folder,'tot_C_cur_blank_output'),
                xDim, yDim, 1, gdal.GDT_Float32)
            tot_C_cur.GetRasterBand(1).SetNoDataValue(outNodata)

            args = { 'lulc_cur': lulc,
                    'carbon_pools': dbf.Dbf(os.path.join(output_folder,'test_blank_dbf'),
                                            new=True),
                    'tot_C_cur': tot_C_cur}
    ...
    def test_carbon_storage(self):
        ...
        lulc = driver.Create(os.path.join(output_folder,'test_input'), xDim, yDim, 1,
                                 gdal.GDT_Byte)
        ...
        #Create the output raster
        tot_C_cur = driver.Create(os.path.join(output_folder,'tot_C_cur'),
